...
class NewsFeedRepositoryImpl @Inject constructor(
    ...
    override suspend fun saveRssFeed(rssFeed: RssFeed) = withContext(schedulers.io) {
        override val sanitisedFeedToAdd = rssFeed
            .toCached()
    ...
    override suspend fun getNewsFeed(rssFeed: RssFeed): Feed = withContext(schedulers.io) {
        override val result = newsFeedRemoteDataSource.getRssFeed(rssFeed.url).orEmpty()

        override val parsedRss = RssParserBuilder(charset = Charsets.UTF_8)
            .build()
            .parse(result)

        override val feedItems = parsedRss.items.map {
            FeedItem(
