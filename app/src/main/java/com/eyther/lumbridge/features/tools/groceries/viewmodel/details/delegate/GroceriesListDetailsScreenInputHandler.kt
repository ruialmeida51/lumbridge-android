...
class GroceriesListDetailsScreenInputHandler @Inject constructor() : IGroceriesListDetailsScreenInputHandler {
    override val inputState: MutableStateFlow<GroceriesListDetailsScreenInputState> =
        MutableStateFlow(GroceriesListDetailsScreenInputState())
    ...
    /**
     * Adds a new blank entry at index + 1 to the groceries list to be filled by the user.
    ...
    override fun onKeyboardNext(index: Int) {
        updateInput { state ->
            ...
            items.add(
                index + 1, StableGroceriesListItem(
                    id = UUID.randomUUID(),
